...
fn test_only_owner_can_propose_ownership_transfer() {
    ...
    let res = propose_new_owner(&mut app, &mock.nft_contract, &bad_guy, &bad_guy);
...
fn test_propose_ownership_stores() {
    ...
    let mock = mock_env(&mut app, &original_owner);

    let new_owner = Addr::unchecked("new_owner");
    propose_new_owner(&mut app, &mock.nft_contract, &original_owner, &new_owner).unwrap();

    let pending_owner_in_storage = query_pending_owner(&app, &mock.nft_contract).unwrap();
...
fn test_proposed_owner_can_accept_ownership() {
    ...
    accept_proposed_owner(&mut app, &mock.nft_contract, &new_owner).unwrap();

    let res = query_pending_owner(&app, &mock.nft_contract);
    if res.is_ok() {
...
fn test_only_proposed_owner_can_accept() {
    ...
    let bad_guy = Addr::unchecked("bad_guy");
    let res = accept_proposed_owner(&mut app, &mock.nft_contract, &bad_guy);
    if res.is_ok() {
