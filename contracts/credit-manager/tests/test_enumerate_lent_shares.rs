...
#[test]
fn test_pagination_on_all_lent_shares_query_works() {
    ...
    let all_lent_shares_res = mock.query_all_lent_shares(None, Some(58_u32));

    // Assert maximum is observed
    assert_eq!(all_lent_shares_res.len(), 30);

    let all_lent_shares_res = mock.query_all_lent_shares(None, Some(2_u32));

    // Assert limit request is observed
    assert_eq!(all_lent_shares_res.len(), 2);

    let all_lent_shares_res_a = mock.query_all_lent_shares(None, None);

    let SharesResponseItem {
        account_id,
        denom,
        ..
    } = all_lent_shares_res_a.last().unwrap().clone();
    let all_lent_shares_res_b = mock.query_all_lent_shares(Some((account_id, denom)), None);

    let SharesResponseItem {
        account_id,
        denom,
        ..
    } = all_lent_shares_res_b.last().unwrap().clone();
    let all_lent_shares_res_c = mock.query_all_lent_shares(Some((account_id, denom)), None);

    let SharesResponseItem {
        account_id,
        denom,
        ..
    } = all_lent_shares_res_c.last().unwrap().clone();
    let all_lent_shares_res_d = mock.query_all_lent_shares(Some((account_id, denom)), None);

    // Assert default is observed
    assert_eq!(all_lent_shares_res_a.len(), 10);
    assert_eq!(all_lent_shares_res_b.len(), 10);
    assert_eq!(all_lent_shares_res_c.len(), 10);

    assert_eq!(all_lent_shares_res_d.len(), 2);

    let combined_res: Vec<SharesResponseItem> = all_lent_shares_res_a
        .iter()
        .cloned()
        .chain(all_lent_shares_res_b.iter().cloned())
        .chain(all_lent_shares_res_c.iter().cloned())
        .chain(all_lent_shares_res_d.iter().cloned())
        .collect();
