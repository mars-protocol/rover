...
fn test_allowed_vaults_set_on_instantiate() {
    ...
    let allowed_vaults = vec![
        VaultBase("vaultcontract1".to_string()),
        VaultBase("vaultcontract2".to_string()),
        VaultBase("vaultcontract3".to_string()),
    ];
    ...
    let vaults_res: Vec<VaultUnchecked> = app
        .wrap()
        .query_wasm_smart(
            contract_addr,
...
fn test_raises_on_invalid_vaults_addr() {
    ...
    let msg = InstantiateMsg {
        owner: owner.to_string(),
        allowed_vaults: vec![VaultBase("%%%INVALID%%%".to_string())],
        allowed_coins: vec![],
        red_bank: RedBankBase::new("red_bank_contract".to_string()),
        oracle: OracleBase::new("oracle_contract".to_string()),
    };
