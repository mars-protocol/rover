...
use crate::helpers::{mock_oracle_contract, mock_vault_contract, CoinInfo, VaultInfo};
...
pub fn setup_credit_manager(
    ...
    allowed_vaults: Vec<VaultUnchecked>,
) -> MockEnv {
...
pub fn deploy_vault(app: &mut App, vault: VaultInfo, oracle: Addr) -> Vault {
...
pub fn fund_vault(app: &mut BasicApp, vault: Vault, funds: Vec<Coin>) {
    app.sudo(SudoMsg::Bank(BankSudo::Mint {
        to_address: vault.0.to_string(),
        amount: funds,
    }))
    .unwrap();
}
