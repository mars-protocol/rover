...
pub static DEFAULT_DEBT_SHARES_PER_COIN_BORROWED: Uint256 = Uint256::from(1_000_000u128);
...
pub fn borrow(
    ...
    let debt_shares_to_add = if total_debt_amount.is_zero() {
        Uint256::from(coin.amount).checked_mul(DEFAULT_DEBT_SHARES_PER_COIN_BORROWED)?
    } else {
        TOTAL_DEBT_SHARES
    ...
    increment_coin_balance(deps.storage, account_id, &coin.clone())?;

    Ok(Response::new()
        .add_message(red_bank.borrow_msg(&coin.try_into()?)?)
