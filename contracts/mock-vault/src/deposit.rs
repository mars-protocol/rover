...
use crate::contract::STARTING_SHARES;
use crate::error::ContractError;
use crate::query::get_all_vault_assets;
...
pub fn deposit(deps: DepsMut, info: MessageInfo) -> Result<Response, ContractError> {
    ...
    let all_vault_assets = get_all_vault_assets(deps.storage)?;

    let shares_to_add = match total_shares_opt {
        None => {
            TOTAL_VAULT_SHARES.save(deps.storage, &STARTING_SHARES)?;
            STARTING_SHARES
        }
        Some(total_shares) => {
