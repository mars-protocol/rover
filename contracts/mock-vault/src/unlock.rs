...
use crate::state::{LOCKUP_TIME, NEXT_UNLOCK_ID, UNLOCKING_TOKENS};
...
pub fn request_unlock(
    ...
    UNLOCKING_TOKENS.update(deps.storage, info.sender, |opt| -> StdResult<_> {
        let mut unlocking_positions = opt.unwrap_or_default();
        unlocking_positions.push(UnlockingTokens {
            id: next_unlock_id,
...
pub fn unlock(
    ...
    let unlocking_positions = UNLOCKING_TOKENS
