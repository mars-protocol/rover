...
use crate::state::{LOCKUP_TIME, NEXT_UNLOCK_ID, UNLOCKING_COINS, UNLOCK_REQUEST_QUEUE_TIME};
...
pub fn request_unlock(
    ...
    let request_queue_time_opt = UNLOCK_REQUEST_QUEUE_TIME.load(deps.storage)?;
    ...
    let unlocked_at = env
        .block
        .time
        .plus_seconds(lockup_time)
        .plus_seconds(request_queue_time_opt.unwrap_or(0));
