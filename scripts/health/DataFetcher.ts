...
import {AccountKind, HealthValuesResponse,} from '../types/generated/mars-rover-health-types/MarsRoverHealthTypes.types'
import {
  DenomsData,
  HealthComputer,
  VaultsData,
...
import {BorrowTarget, compute_health_js, max_borrow_estimate_js, max_withdraw_estimate_js,} from './pkg-web'
...
export class DataFetcher {
    ...
    fetchDenomsData = async (positions: Positions): Promise<DenomsData> => {
        ...
        const lendDenoms = positions.lends.map((c) => c.denom)

        const allDenoms = depositDenoms.concat(debtDenoms).concat(vaultBaseTokenDenoms).concat(lendDenoms)
