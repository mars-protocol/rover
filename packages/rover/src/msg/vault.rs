...
pub enum ExecuteMsg {
    // TODO: If uneven amounts sent, vault replies with `refunded_coins` event
...
pub enum QueryMsg {
    ...
    /// Returns `Vec<UnlockingPosition>` representing the vault `Coin` that this address has requested to unlock
    UnlockingPositionsForAddr { addr: String },
    /// Returns `UnlockingPosition` associated with position id
...
pub struct VaultInfo {
    ...
    /// Time in seconds until an unlock request is registered
